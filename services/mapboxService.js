...
async function createGeoJSONFile(data) {
  ...
  await fs.promises
    .writeFile(outputFilePath, lineDelimitedData, "utf8")
    .then(() =>
      logger.info(`Line-delimited GeoJSON created at ${outputFilePath}`)
    )
...
async function updateTilesetSource(filePath) {
  ...
  try {
    ...
    logger.error(
      `Error updating tileset source: ${error}, ${JSON.stringify(
        error.response.data
      )}`
    );
  } finally {
    ...
    if (tilesets.data && tilesets.data.length > 0) {
      if (
        tilesets.data.some(
          (tileset) =>
            tileset.id ===
            `${MAPBOX_CONFIG.username}.${MAPBOX_CONFIG.tilesetId}`
        )
      ) {
