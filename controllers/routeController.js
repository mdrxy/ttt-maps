...
router.post("/geocode", async (req, res) => {
  ...
  try {
    ...
    if (geoCoordinates) {
      logger.info(
        `Coordinates for ${airtableData.schoolName}: ${JSON.stringify(
          geoCoordinates
        )}`
      );
    ...
    res.status(500).json({
      error: "ServerError",
      message:
        "An unexpected error occurred during geocoding. Please try again later.",
    });
  }
});
...
router.get("/update", async (req, res) => {
  res
    .status(200)
    .send(
      "Update process initialized (does not mean it is complete). Check logs for more information."
    );
